/**
* Adding smallpaint_fixed.cpp implementation by karoly zsolnai into an opengl application.
* 
...
namespace glm
{
  // 4fun: creating a new type of glm vector with GLubyte
	typedef detail::tvec4<GLubyte, precision::highp> highp_glubvec4_t;
	typedef highp_glubvec4_t glubvec4;
};

const int width = 900, height = 900;
int MAX_DEPTH = 4; 
glm::glubvec4 clear_color = glm::glubvec4(0, 0, 0, 255);
glm::glubvec4 pixels[width * height];
bool redisplay = true;
		...
		for (int col = 0; col < width; col++) {
			...
			for (int row = 0; row < height; row++) {
				...
				for (int s = 0; s < spp; s++) {
					...
					pixel = pixel + color / spp; // write the contributions
				}
				pixels[(height - (row + 1)) * width + col] = glm::min(glm::ivec4(pixel.x, pixel.y, pixel.z, 255), 255);
			}
		}
		fprintf(stdout, "\rRendering: %1.0fspp %8.2f%%", spp, (double)100);

		clock_t end_clock = clock();
    std::cout << "Time elapsed: " << (double)(end_clock - start_clock)/(double)(CLOCKS_PER_SEC) << std::endl;
